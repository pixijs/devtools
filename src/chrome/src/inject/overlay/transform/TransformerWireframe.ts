import type { Graphics, Matrix, Point } from 'pixi.js';
...
import { AxisAlignedBounds } from './bounds/AxisAlignedBounds';
...
export function getWireframe(): new (t: any)=> any {
   ...
   */
  return class TransformerWireframe extends getPixiObject('Graphics') {
    ...
    constructor(transformer: Transformer) {
      ...
      const Graphics = getPixiObject('Graphics');
      this.boxScalingHandles = [
        this.addChild(new Graphics()),
        this.addChild(new Graphics()),
        this.addChild(new Graphics()),
        this.addChild(new Graphics()),
      ] as unknown as [Graphics, Graphics, Graphics, Graphics];
    ...
    public hitHandleType(groupBounds: OrientedBounds, projectionTransform: Matrix, pointerPosition: Point): Handle {
      const { boxRotationEnabled, boxRotationTolerance, boxScalingEnabled, boxScalingTolerance } = this.transformer;
    ...
    public drawBoxScalingTolerance(
      ...
      // Inner four corners
      const innerHull = [new Point(), new Point(), new Point(), new Point()];
      ...
      // Outer four corners
      const outerHull = [new Point(), new Point(), new Point(), new Point()];
    ...
    public drawBoxRotationTolerance(): void {
      ...
      const matrix = tempMatrix.identity().scale(t, t).rotate(this.transformer.getGroupBounds().rotation);
