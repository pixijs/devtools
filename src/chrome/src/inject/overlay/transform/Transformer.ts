import type { Container, Matrix, Point } from 'pixi.js';
import { getHandle } from './TransformerHandle';
import { getWireframe } from './TransformerWireframe';
import { OrientedBounds } from './bounds/OrientedBounds';
...
import type { Cursor, FederatedPointerEvent } from 'pixi.js';
import type { ITransformerHandleStyle } from './TransformerHandle';
import { getPixiObject } from './utils/getPixiObject';
...
export interface ITransformerOptions {
  ...
  handleConstructor: typeof getHandle;
...
export function getTransformer() {
  ...
  const Point = getPixiObject('Point');
  const Transform = getPixiObject('Transform');
  const Matrix = getPixiObject('Matrix');
  const Rectangle = getPixiObject('Rectangle');
   ...
   */
  return class Transformer extends getPixiObject('Container') {
    ...
    protected handles: { [H in Handle]: InstanceType<ReturnType<typeof getWireframe>> };
    ...
    protected wireframe: InstanceType<ReturnType<typeof getWireframe>>;
    ...
    private _pointerMoveTarget: Container;
    lazyDirty: boolean;
    ...
    constructor(options: Partial<ITransformerOptions> = {}) {
      ...
      this.rotationSnapTolerance =
        options.rotationSnapTolerance !== undefined ? options.rotationSnapTolerance : DEFAULT_ROTATION_SNAP_TOLERANCE;

      this.skewRadius = options.skewRadius || 64;
      this.skewSnaps = options.skewSnaps || DEFAULT_SKEW_SNAPS;

      this.skewSnapTolerance =
        options.skewSnapTolerance !== undefined ? options.skewSnapTolerance : DEFAULT_SKEW_SNAP_TOLERANCE;
      ...
      this.wireframe = this.addChild(new getWireframe()(this));
      ...
      const HandleConstructor = options.handleConstructor || getHandle();
      ...
      this.handles = Object.assign({}, scaleHandles, rotatorHandles, skewHandles) as {
        [H in Handle]?: getHandle();
      };
    ...
    translateGroup = (delta: Point): void => {
      ...
      // Translation matrix
      const matrix = tempMatrix.identity().translate(delta.x, delta.y);
    ...
    skewGroup = (handle: SkewHandle, pointerPosition: Point): void => {
      ...
      // Skew matrix
      const matrix = tempMatrix.identity().translate(-sOrigin.x, -sOrigin.y);
     ...
     */
    render(): void {
      if (this.renderable && this.visible && this.lazyDirty) {
        this.draw();
      }
    }
    ...
    protected draw(): void {
      ...
      if (this.boxRotationEnabled) {
        this.wireframe.closePath().beginFill(0xffffff, 1e-4).lineStyle();
        this.wireframe.drawBoxRotationTolerance();
      }

      if (this.boxScalingEnabled) {
        this.wireframe.closePath().beginFill(0xfff0ff, 1e-4).lineStyle();
    ...
    protected drawHandles(groupBounds: OrientedBounds): void {
      ...
      if (this._rotateEnabled) {
        ...
        this.wireframe.moveTo(bx, by).lineTo(handles.rotator.position.x, handles.rotator.position.y);
      ...
      if (this._skewEnabled) {
        ...
        handleAnchors.skewVertical.set(
    ...
    protected onPointerMove(e: FederatedPointerEvent): void {
      ...
      const currentPointerPosition = new Point().copyFrom(e.data.global);
     ...
     * @param handle1
     */
    private swapHandles(handle0, handle1): void {
     ...
     */
    static calculateOrientedBounds(displayObject: Container, bounds?: OrientedBounds): OrientedBounds {
      ...
      // Unrotation matrix
      const matrix = tempMatrix.identity().translate(-cx, -cy).rotate(-tempTransform.rotation).translate(cx, cy);
      ...
      bounds.innerBounds.width =
        Math.max(corners[0].x, corners[1].x, corners[2].x, corners[3].x) - bounds.innerBounds.x;
      bounds.innerBounds.height =
        Math.max(corners[0].y, corners[1].y, corners[2].y, corners[3].y) - bounds.innerBounds.y;
    ...
    static calculateGroupOrientedBounds(
      ...
      const frames = [new Point(), new Point(), new Point(), new Point()];
      ...
      // Unrotation matrix
      const matrix = tempMatrix.identity().rotate(-rotation);
