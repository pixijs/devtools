...
// @ts-nocheck - This is a port of the diff library from the `diff` package
...
interface DiffCallback {
  (oldString?: string, newString?: string, options?: DiffOptions): any;
}
...
export default class Diff {
  ...
  diff(oldString: string, newString: string, options: DiffOptions = {}): any {
  ...
  addToPath(path: Path, added?: boolean, removed?: boolean, oldPosInc = 0): Path {
  ...
  for (; componentPos < componentLen; componentPos++) {
    ...
    if (!component.removed) {
      ...
      if (componentPos && components[componentPos - 1].added) {
        const tmp = components[componentPos - 1] as Component;
