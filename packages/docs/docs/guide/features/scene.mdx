...
The Scene Panel is a powerful tool that allows you to inspect and manipulate the scene graph of your PixiJS application in real-time.
...
:::info
Installing `@pixi/devtools` will automatically add the `__devtoolIgnore` and `__devtoolIgnoreChildren` types
:::
